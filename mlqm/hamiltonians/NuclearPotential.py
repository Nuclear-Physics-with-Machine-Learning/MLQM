...
class NuclearPotential(Hamiltonian):
    ...
    # @tf.function()
    # @tf.function(experimental_compile=False)
    def compute_energies(self, inputs, w_of_x, dw_dx, d2w_dx2):
        ...
        ke_jf = self.kinetic_energy_jf(w_of_x=w_of_x, dw_dx=dw_dx, M=self.parameters["mass"])

        # True, directly, uses the second derivative
        ke_direct = self.kinetic_energy(w_of_x=w_of_x, d2w_dx2 = d2w_dx2, M=self.parameters["mass"])
